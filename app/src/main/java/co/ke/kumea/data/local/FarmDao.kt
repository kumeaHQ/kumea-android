...
interface FarmDao {
    ...
    /** Single active farm by id — pending OR synced (a pendingSync-only lookup misses synced rows). */
    @Query("SELECT * FROM farms WHERE id = :id AND deletedAt IS NULL AND syncAction != 'DELETE' LIMIT 1")
