...
data class RegisterRequest(
    ...
    val pin: String,
    // Build-1 (11 Jul): the person's name. The app never asked who the farmer
    // IS — a user was id+phone+role. Required by the client UI; optional on
    // the wire so older APKs remain valid against the new server.
...
data class AuthUser(
    ...
    val phone: String,
    val role: String,
    // Optional: pre-Build-1 servers/users won't send it; never crash on absence.
    val name: String? = null,
)
...
data class UserProfile(
    ...
    val phone: String,
    val role: String,
    // Optional: pre-Build-1 users have no name until backfilled.
    val name: String? = null,
