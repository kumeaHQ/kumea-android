...
interface KumeaApi {
    ...
    // ---- Harvests (Build-2) ----
    // Flat /harvests resource, identical sync shape to /fields. fieldId travels
    // in the create body. Quantities are decimal STRINGS end-to-end ("1.2") —
    // the acres/money rule; never a JSON number.
    ...
    @POST("harvests")
    suspend fun createHarvest(@Body harvest: HarvestCreateRequest): Response<HarvestResponse>

    @PATCH("harvests/{id}")
    suspend fun updateHarvest(
        @Path("id") id: String,
        @Body harvest: HarvestUpdateRequest,
